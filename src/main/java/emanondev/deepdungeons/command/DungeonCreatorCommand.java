...
public class DungeonCreatorCommand extends CoreCommand {
            ...
            case "test" -> {
                ...
                this.run = new BukkitRunnable() {
                    ...
                    public void run() {
                        ...
                        currentLoc.setWorld(null);
                        BoundingBox box = p.getBoundingBox();
                        Location boundingBoxLoc = new Location(null, box.getCenterX(), box.getMinY(), box.getCenterZ());
                        Location diff = currentLoc.subtract(boundingBoxLoc);
                        Bukkit.broadcastMessage("Diff X: " + diff.getX() + " Y: " + diff.getY() + " Z: " + diff.getZ());
                    }
                }.runTaskTimer(DeepDungeons.get(), 20L, 20L);
                return;
            }
        }
        sender.sendMessage("Help Message not implemented");
    ...
    private void start(CommandSender sender, String label, String[] args) {
        ...
        PartyManager.getInstance().startDungeon(party, handler, List.of(player));
    }
