...
public class TrapTypeManager extends DRegistry<TrapType> {
    ...
    @Nullable
    public TrapType getTrapType(@NotNull ItemStack itemStack) {
    ...
    @Nullable
    public TrapType getTrapType(@NotNull ItemMeta meta) {
    ...
    @Nullable
    public TrapInstanceBuilder getTrapInstance(@NotNull ItemStack itemStack) {
        if (itemStack.getType() != Material.PAPER || !itemStack.hasItemMeta())
            return null;
    ...
    @Nullable
    public TrapInstanceBuilder getTrapInstance(@NotNull ItemMeta meta) {
        if (!meta.hasLore() || !LINE_ONE.equals(meta.getDisplayName()))
            return null;
        List<String> lore = meta.getLore();
        if (lore.size() == 0)
